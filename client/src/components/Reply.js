...
class Reply extends React.Component {
    ...
    componentDidMount() {
        axios.get("/users/" + this.state.reply.user_id +"/profile")
            .then(response => {
                this.setState({ fetchedProfile: true, profile: response.data })
            })
            .catch(error => {
                console.log(error)
                this.setState({ fetchedProfile: true, profile: { image: { path: "" }, family_name: "", given_name: "" } });
            })
    }
    handleDelete = () => {
        const announcementId = this.state.reply.announcement_id;
        const replyId = this.state.deleteId;
        axios.delete("/groups/" + this.props.groupId + "/announcements/" + announcementId + "/replies/" + replyId)
            .then(response => {
