...
class FramilyListItem extends React.Component {
    state = { fetchedProfile: false, child: {}, modalIsOpen: false, right: 0, top: 0};
    componentDidMount() {
        axios.get("/users/" + this.props.framilyId + "/profile")
            .then(response => {
								const profile = response.data;
								const myProfile = JSON.parse(localStorage.getItem("user")).id===this.props.profileId
		...
		mouseDown = (event) => {
			this.setState({ timer: new Date().getTime(), right: "5%",top: event.clientY })
		} 
		mouseUp = () => {
		...
		touchDown = (event) => {
			this.setState({ timer: new Date().getTime(), right: "5%",top: event.touches[0].clientY })
		} 
		handleDeleteFramily = () => {
                ...
                {this.state.fetchedProfile ?
                    <React.Fragment>
                        <div className="col-3-10">
                          <Avatar thumbnail={profile.image.path} route={route} className="horizontalCenter" />
                        </div>
