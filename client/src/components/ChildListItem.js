...
class ChildListItem extends React.Component {
    ...
    componentDidMount() {
        axios.get("/users/" + this.props.userId + "/children/" + this.props.childId)
            .then(response => {
                const child = response.data;
                this.setState({ fetchedChild: true, child: child })
            })
            .catch(error => {
                console.log(error);
                this.setState({
    ...
    render() {
        ...
        const route = this.props.history.location.pathname + "/" + this.props.childId;
        return (
            <div id="childContainer" className="row no-gutters" style={{ borderBottom: "1px solid rgba(0,0,0,0.1" }}>
