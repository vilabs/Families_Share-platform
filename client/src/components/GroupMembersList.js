...
export default class GroupMembersList extends React.Component {
	...
	componentDidMount() {
		...
		axios
			...
			.then(response => {
				...
				this.setState({ fetchedUsers: true, users: users });
			})
			.catch(error => {
				console.log(error);
	...
	handleRemoveUser = (userId) => {
		this.setState({ users: this.state.users.filter(user=> user.user_id !== userId )})
	}
	renderLetters = () => {
		...
		const sortedMembers = []
			...
			.sort(
				(a, b) => {
					if (`${a.given_name} ${a.family_name}` < `${b.given_name} ${b.family_name}`) { return -1; }
					else { return 1; }
				});
