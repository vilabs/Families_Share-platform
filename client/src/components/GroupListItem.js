...
class GroupListItem extends React.Component {
  ...
  async componentDidMount() {
    ...
    group.members = members.filter(member => member.user_accepted && member.group_accepted );
  ...
  render() {
    ...
    return (
      <div className="row no-gutters" id="suggestionContainer" onClick={this.handleNavigation}>
        {this.state.fetchedGroupData ?
          <React.Fragment>
            <div className="col-2-10">
              <Avatar thumbnail={group.image.thumbnail_path} className={"center"} route={"/groups/" + group.group_id + "/activities"} />
            </div>
