...
class AnnouncementReplies extends React.Component {
    ...
    componentDidMount() {
        ...
        axios.get("/groups/" + groupId + "/announcements/" + announcementId + "/replies")
            .then(response => {
                const replies = response.data;
                this.setState({ fetchedReplies: true, replies: replies })
            })
            .catch(error => {
                console.log(error);
    ...
    handleSend = () => {
        const groupId = this.props.groupId;
        const announcementId = this.props.announcementId;
        axios.post("/groups/" + groupId + "/announcements/" + announcementId + "/replies", {
    ...
    render() {
        ...
        const showRepliesIcon = this.state.showReplies ? "fas fa-chevron-up" : "fas fa-chevron-down"
        return (
            <React.Fragment>
                ...
                <div id="announcementRepliesContainer" style={this.state.showReplies ? {} : { display: "none" }}>
                    ...
                    <div className="row no-gutters" id="newReplyContainer">
                        <div className="col-8-10">
                            <input type="text" placeholder={texts.new} value={this.state.newReply}
                                onChange={this.handleChange} className="verticalCenter" onKeyUp={this.handleEnter}
                            />
                        </div>
                        <div className="col-2-10">
                            <button className="transparentButton center" onClick={this.handleSend}>
