...
router.post('/forgotpassword', (req, res, next) => {
  ...
  User.findOne({ email }, (error, user) => {
    if (error) next(error);
    if (user) {
      const token = jwt.sign({ user_id: user.user_id, email }, process.env.SERVER_SECRET, { expiresIn: 60 * 60 * 24 });
      const mailOptions = {
      ...
      transporter.sendMail(mailOptions, (err, info) => {
        if (err) next(err);
        Password_Reset.findOne({ user_id: user.user_id, email }, (er, reset) => {
...
router.get('/changepassword', async (req, res, next) => {
  ...
  try {
    const { user_id } = req;
    const reset = await Password_Reset.findOne({ user_id });
    if (reset) {
...
router.patch('/:userId/groups/:groupId', (req, res, next) => {
  ...
  if (user_id && group_id && patch.user_accepted !== undefined) {
    Member.updateOne({ user_id, group_id }, patch, async (err, raw) => {
      if (err) next(err);
